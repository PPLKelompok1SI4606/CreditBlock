            ...
            <div class="relative bg-white rounded-lg p-6 overflow-hidden">
                ...
                <div class="flex items-center space-x-4 mb-4">
                    ...
                    <div>
                        ...
                        <!-- Kode yang sudah ada untuk total pinjaman -->
                        @php
                ...
                <div class="mt-2 space-y-3">
                    <!-- Kode yang sudah ada untuk cicilan berikutnya -->
                    ...
                    <p class="text-gray-600 text-sm flex items-center">
                        ...
                        <span class="inline-block bg-green-100 text-green-700 text-xs font-medium px-2.5 py-1 rounded-full ml-2">
                            {{ $loan ? $loan->status : 'Tidak Ada' }}
                        </span>
                    </p>

                    <!-- Tambahan: Card Pinjaman Saya (duplikat) -->
                    <!-- Card Pinjaman Saya -->
                    @php
                        $loan = \App\Models\LoanApplication::where('user_id', Auth::id())->where('status', 'Aktif')->first();
                        $remainingAmount = $loan ? ($loan->amount - $loan->payments()->sum('amount')) : 0;
                ...
                <!-- Action Buttons -->
                <div class="mt-6 flex items-center space-x-4">
                    <a href="{{ route('payment.create') }}" class="bg-blue-500 text-white px-5 py-2 rounded-lg font-medium text-sm tracking-wide transition-all duration-300 hover:bg-blue-600 hover:ring-2 hover:ring-blue-200 hover:ring-opacity-50">
                        Bayar Cicilan
                    </a>
        ...
        <div class="relative bg-white rounded-lg p-6 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-gray-700 text-sm">
                    ...
                    <!-- Kode yang sudah ada untuk tabel riwayat pembayaran -->
                    @php
                        $payments = \App\Models\Payment::where('user_id', Auth::id())->orderBy('payment_date', 'desc')->take(3)->get();
                    @endphp
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500">Belum ada riwayat pembayaran.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

                <!-- Tambahan: Tabel Riwayat Pembayaran (duplikat) -->
                <table class="w-full text-gray-700 text-sm mt-6">
                    <thead>
                        <tr class="bg-gray-50 text-gray-900">
                            <th class="px-6 py-4 text-left rounded-tl-lg font-medium tracking-wide">Tanggal</th>
                            <th class="px-6 py-4 text-left font-medium tracking-wide">Nominal</th>
            ...
            <div class="mt-6 flex items-center space-x-6">
                <!-- Kode yang sudah ada untuk link "Lihat Semua" -->
                <a href="{{ route('payment.history') }}" class="relative text-blue-500 text-sm font-medium tracking-wide transition-all duration-300 hover:text-blue-600 group">
                    Lihat Semua
                    <span class="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></span>
                </a>
                <a href="#" class="relative text-blue-500 text-sm font-medium tracking-wide transition-all duration-300 hover:text-blue-600 group">
                ...
                <!-- Tambahan: Link "Lihat Semua" (duplikat) -->
                <a href="{{ route('payment.history') }}" class="relative text-blue-500 text-sm font-medium tracking-wide transition-all duration-300 hover:text-blue-600 group">
                    Lihat Semua
                    <span class="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></span>
                </a>
